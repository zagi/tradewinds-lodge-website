...
export function getAllMarkdownFilesInDirectory(
...
export async function getAllMarkdownData<T extends BaseContent>(
  directoryPath: string,
): Promise<T[]> {
  ...
  const allMarkdownData = await Promise.all(
    files.map(async (fileName) => {
      const { frontMatter, content, slug } = await getMarkdownContent<
        Omit<T, "slug" | "content">
...
export async function getMarkdownDataBySlug<T extends BaseContent>(
  directoryPath: string,
  slug: string,
): Promise<T | null> {
  const files = getAllMarkdownFilesInDirectory(directoryPath);
